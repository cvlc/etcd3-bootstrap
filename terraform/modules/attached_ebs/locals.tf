  ...
  user_data_snippets_by_az = {
    ...
    az => join("\n", [
      for key, value in var.attached_ebs : value["availability_zone"] == az ? templatefile("${path.module}/cloudinit/userdata-snippet.sh", {
        ...
        ebs_bootstrap_unit = templatefile("${path.module}/cloudinit/ebs_bootstrap_unit.tftpl", {
          region                   = data.aws_region.current.name
          ...
          ebs_volume_name          = "${key}-${value["availability_zone"]}"
          block_device_aws         = contains(keys(value), "block_device_aws") == true ? value["block_device_aws"] : ""
          block_device_os          = value["block_device_os"]
          block_device_mount_path  = value["block_device_mount_path"]
          ephemeral                = lookup(value, "ephemeral", false) ? value["ephemeral"] : false
